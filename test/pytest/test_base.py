...
from instrumentserver.base import encode, decode, send, recv, sendBroadcast, recvMultipart
...
def test_encode_broadcast_blueprint_returns_string():
    bp = ParameterBroadcastBluePrint(name='p', action='parameter-update', value=42, unit='V')
    result = encode(bp)
...
def test_send_recv_broadcast_blueprint(zmq_pair):
    s1, s2 = zmq_pair
    bp = ParameterBroadcastBluePrint(name='my_p', action='parameter-update', value=7, unit='Hz')
    send(s1, bp)
    result = recv(s2)
...
def test_sendBroadcast_recvMultipart(zmq_pub_sub):
    pub, sub = zmq_pub_sub
    bp = ParameterBroadcastBluePrint(name='my_param', action='parameter-update', value=7, unit='V')
